...
- The hidden admin console now includes live diagnostics, scenario/preset buttons, event triggers, shell toggles, and a command terminal (`status`, `grant`, `upgrade`, `spawn`, `event`, `heal`, `clear`, `preset`, and more) for QA and balance setup; it can collapse into a tiny quick-send command panel when you need the field visible
...
| `src/components/AdminPanel.tsx`           | Hidden admin console opened by Space × 5: diagnostics, quick actions, shell toggles, event trigger shortcuts, command-terminal UI, and a collapsed quick-send command panel                                                                          |
