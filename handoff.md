...
- **3.1.3 — dev build indicator.** `npm run dev` boots with `import.meta.env.DEV === true`, which (a) renders an amber `BETA` pill next to the version button in `App.tsx`, (b) rewrites every `<link rel*="icon">` href in `main.tsx` to its `-dev` variant (currently only `nexus-drift-dev.svg` ships; missing files degrade gracefully — browsers fall through to the next icon link), and (c) prefixes the document title with `[DEV]`. Production builds (`npm run build && npm run preview`) are unaffected. If you add a new favicon size, drop the `-dev` variant alongside it for the swap regex to pick up.
