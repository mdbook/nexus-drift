...
- `src/components/AdminPanel.tsx` — hidden admin console opened with Space × 5 (keyboard) or by tapping the version badge 5 times within 2 s (mobile). Single Card that animates height via CSS `grid-template-rows` when collapsing/expanding; chevron toggle rotates 180° and renders as a tab bulge that visually merges with the panel's top border (rounded-t-full + opaque cover strip hiding the card border line across the bulge width). When expanded, the body is vertically resizeable via a grip handle at the top of the expanded section (pointer drag, clamped 200 px – 85 dvh, default 460 px). The two-column body is decoupled: the left column (diagnostics / quick actions / shell / event triggers) scrolls independently while the right command terminal column always fills the full panel height and scrolls only its own entries. The terminal entries area uses `min-h-0 flex-1` so the command input form stays visible at any panel height. The console is app-shell/UI state only; command history, collapsed state, and resize height are not persisted.
