...
- **3.1.3 — dev build indicator.** `npm run dev` boots with `import.meta.env.DEV === true`, which (a) renders an amber `BETA` pill next to the version button in `App.tsx`, (b) swaps only the `<link rel*="icon" type="image/svg+xml">` href in `main.tsx` from `nexus-drift.svg` → `nexus-drift-dev.svg` (raster icons stay pointed at production assets so there are no 404s; SVG-capable browsers render the tinted icon and older browsers fall back to the prod raster icon), and (c) prefixes the document title with `[DEV]`. Production builds (`npm run build && npm run preview`) are unaffected. If you want fully-branded raster dev icons in the future, ship `-dev` variants alongside each raster asset and widen the regex to match them.
