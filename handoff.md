...
Current version: **3.2.4**. The in-game changelog is at `src/changelog.ts` and opens via the version badge in the header. As of 2.0.0 the project dropped its leading `0.` prefix from all historical versions — the first release is now `0.1.0` (was `0.0.1`), and the "Living Field" milestone is `2.0.0` (was `0.2.0`).
...
- Save files carry a `schemaVersion` field (currently `12`). `migrateGameState()` always stamps the current version on load and handles older saves gracefully. The `SCHEMA_VERSION` constant lives in `factories.ts`; schema 6 existed only during 3.0.0 branch testing and uses the same field-presence fallback path. v5 added AI fields: `ResourceNode.workTicks`, `Agent.threatMemory`, `Enemy.archetype`, `Enemy.squadId`, optional `Enemy.dashTicks` — all populated with `?? default` fallbacks during migration. v7 scaffolds the Balancing & Behavior update: new `warden` enemy kind and `missileLauncher` upgrade in the unions; per-worker `speedMod` / `fearMod` / `harvestBias` variance plus class-ability ticks (`overclockTicks`, `sprintTicks`, `sprintCooldown`) and corruption fields (`corrupted`, `corruptionTicks`, `corruptingTicks`, `spottedTicks`, `rebootTicks`); turret HP fields (`hp`, `maxHp`, `damageTicks`, `brokenTicks`); scout/sentinel HP + retreat fields (`hp`, `maxHp`, `damageTicks`, `retreating`, `rebootTicks`); new top-level `missileSilos: MissileSilo[]`, `city: CityState`, `nextSiloId` fields; and stats counters `wardensKilled`, `corruptedPurified`, `turretsBroken`. v8 adds `Enemy.permanentCloak?` (warden ghost rework — backfills `true` for wardens on load). v9 adds `Scout.disabledTicks` and `Sentinel.disabledTicks` so zapper bolts can disrupt mobile defenders the same way they already do workers and turrets. v10 adds `Enemy.latchedWorkerId?` (3.1.5 warden parasite latch — persists mid-latch saves; pre-v10 saves default to `null` so the warden resumes roaming). v11 added four 3.2.1 fields together: `state.archiveLog` (long-form 200-entry mirror for upgrade/event/achievement categories), `Agent.spookedTicks` (post-flee threat-aversion memory), and the (now-legacy) `state.achievementToastQueue` + `state.enemyDiscoveryQueue` parallel toast queues plus `state.discoveredEnemies`. v12 (3.2.2) collapses the two parallel queues into a single `state.notifications: Notification[]` (discriminated union of `achievement` / `enemy-discovered`); the migration translates legacy v11 `achievementToastQueue` and `enemyDiscoveryQueue` entries via `buildAchievementNotification` / `buildEnemyDiscoveredNotification` and preserves stable ids for idempotent re-pushes. `state.discoveredEnemies` is unchanged. All new fields carry `?? []` / `?? {}` / `?? 0` migration fallbacks.
...
- `src/components/AdminPanel.tsx` — hidden admin console opened with Space × 5 (keyboard) or by tapping the version badge 5 times within 2 s (mobile). Single Card that animates height via CSS `grid-template-rows` when collapsing/expanding; chevron toggle rotates 180° and renders as a frosted pill that straddles the panel's top edge at its midpoint (rounded-full, `bg-slate-700/35 backdrop-blur-xl`). The pill is a sibling AFTER the Card (outside its backdrop-filter stacking context, so its blur reads the page bg directly), and the Card has a clip-path notch at top-center so the pill's bottom half meets the page bg without stacking slate-on-slate. When expanded, the body is vertically resizeable via a grip handle at the top of the expanded section (pointer drag, clamped 200 px – 85 dvh, default 460 px). The two-column body is decoupled: the left column (diagnostics / quick actions / shell / event triggers) scrolls independently while the right command terminal column always fills the full panel height and scrolls only its own entries. The terminal entries area uses `min-h-0 flex-1` so the command input form stays visible at any panel height. The console is app-shell/UI state only; command history, collapsed state, and resize height are not persisted.
...
- `src/components/ActivityLog.tsx` — structured activity log panel: category icons, relative-age timestamps, filter tabs (including Awards), scrollable history. As of 3.2.1 the panel reads from two arrays: the live 20-entry `state.log` for the "All" tab and combat / mining / corruption / system / ambient filters, and the long-form 200-entry `state.archiveLog` for upgrade / event / achievement filters (which mirror only those archival categories).
- `src/components/NotificationStack.tsx` — 3.2.2 unified bottom-right notification stack. Renders the head three entries of `state.notifications` (cap = `NOTIFICATION_VISIBLE_LIMIT`); additional entries hold in an invisible queue with paused timers and promote automatically as visible slots free up. Per-kind body components route off `notification.kind` (achievement / enemy-discovered) so adding a new toast variant is a three-step contract documented at the top of `src/game/notifications.ts`: extend the union, add a builder, add a render branch. Action callbacks flow through a typed `NotificationAction` discriminated union (`{ kind: "open-wiki"; entryId }`) that the host translates into surface-specific effects. Replaces the 3.2.1 `AchievementToast` (top-right rarity toasts + legendary full-screen flourish) and `EnemyDiscoveryCard` (top-of-field discovery alert) — both are gone, and the legendary full-screen wash is dropped in favor of an amber-toned, longer-dwell stack frame. Discoveries still persist via `state.discoveredEnemies` so a kind never re-triggers after a reload.
...
- `src/game/subsystems/` — economy, spawns (+ stepWardenSpawn), movement, workers (slot activation), corruption, workerCorruption (warden parasite latch + node drain + worker reporting), turrets, scouts, sentinels (+ cleanse path), combat, mining, autobuy, projectiles, events, achievements
- `src/game/__tests__/advanceGame.test.ts` — 152 tests: simulation invariants, subsystem behavior, achievement edge cases, projectile behavior, corruption linger, worker-slot gating/costs, surround-pressure combat, save/load round-trip, turret/scout/sentinel/city HP, multi-class enemy targeting, missile silo subsystem, worker class abilities (Step 6), worker corruption system including warden parasite latch + uncloak + pin / attach cycle / decay, warden kill credit, node drain, worker reporting, sentinel cleanse, worker reboot, stepWardenSpawn gates/cooldown semantics (3.1.5 latch expansion), `xpForLevel` curve pin (HUD/sim parity), and 3.2.1 `archiveLog` routing/cap/migration
...
- `src/game/__tests__/aiBehavior.test.ts` — 30 tests: worker path safety, commitment, flee-direction retargeting, and crowded-node avoidance, archetype targeting, brute target stability, squad bucketing, sentinel intercept priority, scout finish-bias, sticky retarget threshold, ambusher dash trigger/duration, ghost reposition window, group dispersal, save migration, threat-field path weighting, and 3.2.1 worker spook memory (set on evade-end, per-tick decay, reset on `killWorker`, threat-multiplier flip)
- `src/game/__tests__/notifications.test.ts` — 4 tests: unified notification queue idempotence, dismissal, visible-window tick gating, and post-expiry promotion
- `src/game/__tests__/persistence.test.ts` — 4 tests: empty-store cold start, save → load round-trip with key fields preserved, malformed-JSON fallback, and partial-save guard
...
**3.2.0 — defense progression flip.** Turrets are now Tier 3 "Raid" gated (`minTier: 3` on the `turret` upgrade in `data.ts`) and the always-on first-turret floor is gone. Active count: `state.upgrades.turret >= 1 ? min(turrets.length, 1 + min(max(0, upgrades.turret - 1), TURRET_SLOTS_BY_LEVEL[level])) : 0`. So no turret deploys until the player both reaches Tier 3 and buys the upgrade. Subsystems must keep reading `derived.activeTurrets` rather than recomputing locally — `selectors.ts` is the single source of truth. The 2nd turret slot still unlocks at sector level 2, the 3rd at level 8, both gated by upgrade level on top.
...
`MissileSilo` entities (deployed via the `missileLauncher` upgrade track) are separate from turrets and **invulnerable** — they do not carry HP, are not in `ENEMY_TARGET_PRIORITY`, and take no contact damage. Silo count scales with upgrade level via `MISSILE_SILO.silosByLevel` — `[1, 1, 1, 2, 2, 3, 3, 3, 3, 4, 4]` as of 3.2.0. Index 0 is `1`, so a fresh game lands with one silo armed even before the upgrade is purchased. Each active silo fires once per `fireIntervalTicks` (480 ≈ 16s) at the highest-priority combat enemy within `rangeBase + level * rangePerLevel` (400 + 6 × level) — brutes first, then leeches, then everything else, wounded within tier. Target selection is a single-pass best scan, not a sort. Damage is `damageBase (48) + damagePerLevel (12) * level`.

**3.2.0 — defense progression flip.** Silos are now the colony's first defense line. `missileLauncher` has no `minTier` (was 2), its base cost dropped from `{ gold: 2200, cores: 6, flux: 4 }` to `600` gold, and growth eased from `1.32` to `1.30`. `selectors.ts` adds `activeMissileSilos * CITY.developmentWeights.activeTurrets` to `homeDevelopment` so pushing turrets out of the early game does not silently stall city-stage progression — the player's silo investment fills the same role.
...
`stepMissileSilos` runs in `advanceGame` after `stepSentinels` and before `stepZapperFire`. Renderer: chunky orange pylons in `FieldSvg.tsx` with a cooldown charge bar; the range ring is shown at low opacity when the silo is active; a brief launch flash fires when a shot exits.
...
As of 3.2.1 the activity log lives in two arrays. `state.log` is the live HUD feed (capped at `MAX_LOG = 20`, was 40 pre-3.2.1) and shows the most recent moments across every category. `state.archiveLog` is a long-form mirror (capped at `MAX_ARCHIVE_LOG = 200`) that retains only the archival categories — `upgrade`, `event`, `achievement` — so player-facing progression history is not lost when noisy combat / mining chatter scrolls the recent feed. The set of archival categories is exported as `ARCHIVE_LOG_CATEGORIES` from `constants.ts`. Each `LogEntry` carries:
...
`appendLog(state, message, category)` is the sole write path. It pushes onto `state.log` and, when the category is archival, also mirrors the entry into `state.archiveLog`. Every subsystem call site uses this helper; `pushLog` is retained as a low-level slicing primitive but should not be called directly from sim code. `migrateGameState()` handles old saves that stored plain `string[]` entries by mapping them to `{ tick: 0, category: "system", message: entry }`, and v10-and-earlier saves default `archiveLog` to `[]`.
...
- Category filter tab bar: All / Combat / Corrupt / Upgrade / Event / Awards
- Scrollable list (max-h-72) — "All" shows 20 newest entries; Upgrade / Event / Awards swap to the 200-entry archive feed so long-form scroll-back is available without bloating the recent HUD
...
71 achievements across 4 rarity tiers (`common` / `uncommon` / `rare` / `legendary`) and 6 categories (`combat`, `corruption`, `mining`, `progression`, `survival`, `secret`). `AchievementDef` now carries `rarity`, `category`, and an optional `hidden` flag. Hidden locked achievements display as "???" placeholders in the modal until revealed.
...
The achievement ribbon in the field card now uses rarity-coded border/background colours instead of flat indigo. An unlock count badge (e.g. `3/71`) appears at the right end of the strip. Opening the ribbon can itself unlock `archivist` once any hidden secret is already revealed.
...
- Sector-level XP threshold is `xpForLevel(level)` in `balance.ts` — the single source of truth used by both `stepEconomy` (level-up gate) and `computeDerived` (HUD `targetXp`). Do not re-inline the formula in either place. As of 3.2.3 the curve is `floor(30 + L * 15 + L^1.7 * 1.4)` (was the linear `80 + L * 25` through 3.2.2). Old `ECONOMY.levelXpBase` / `ECONOMY.levelXpPerLevel` constants were removed.
...
- **3.1.3/3.1.4 — beta build indicator.** `src/lib/isBetaBuild.ts` is the single gate: returns true when `import.meta.env.DEV === true` (local dev server) or `window.location.hostname` starts with `nexus-drift-beta` (CI-built beta deploys at `nexus-drift-beta.mdbook.me` / `nexus-drift-beta.*.mdbook.one`). When true: (a) renders an amber `BETA` pill next to the version button in `App.tsx`, (b) swaps only the `<link rel*="icon" type="image/svg+xml">` href in `main.tsx` from `nexus-drift.svg` → `nexus-drift-dev.svg` (raster icons stay pointed at production assets; SVG-capable browsers render the tinted icon and older browsers fall back to the prod raster icon), and (c) prefixes the document title with `[BETA]`. Production builds at `nexus-drift.mdbook.me` / `nexus-drift.*.mdbook.one` are unaffected. If you want fully-branded raster dev icons in the future, ship `-dev` variants alongside each raster asset and widen the regex to match them.
...
- `.gitlab-ci.yml` verify stage runs `format:check` + `lint` on top of `typecheck` + `test` — do not drop either step when editing the file.
...
- 213 tests across `src/game/__tests__/advanceGame.test.ts`, `src/game/__tests__/interactionAchievements.test.ts`, `src/game/__tests__/aiBehavior.test.ts`, `src/game/__tests__/adminCommands.test.ts`, `src/game/__tests__/notifications.test.ts`, `src/game/__tests__/persistence.test.ts`, and `src/lib/versionCheck.test.ts` cover simulation invariants, interaction achievements, late-game worker-slot gating, worker unlock resource costs, event HUD linger behavior, event modifier composition (multiplicative stacking survives expiry), late-tier event reachability via uncapped `rawTier`, lone-sapper worker reboot (no hp=0 zombie), selectors filtering hp=0 dying enemies from counts/threats, zapper bolts skipping rebooting workers and broken turrets on impact, colonyHealth normalized as hp/maxHp over active workers (corruption-toughness maxHp=150 stays ≤100), cloneGameState isolating its RNG instance, resolveEnemyDeaths idempotency (tickDeathFades owns the single per-tick decrement), late-game defense scoring (focusedBeam/missileLauncher contribute to defenseScore and weightedUpgradeScore), tick-wrap-safe elapsed delta helper and temporary-node expiry across the TICK_WRAP boundary, AI behavior and archetype targeting, flee-direction worker retargeting, crowded-node avoidance, missile grace behavior, corruption linger, surround-pressure combat, live-version polling helpers, admin preview-version generation, manual-override timing, save/load round-trips and the `loadSavedState` localStorage entry point, multi-class enemy target eligibility, worker corruption + sentinel cleanse (Step 7), warden permanent cloak + 3.1.5 parasite latch (latch-on-contact, uncloak-while-latched, pin-through-worker-movement, release-on-host-death, warden-killed-mid-latch leaves host uncorrupted) + save migration for both cloak and latch fields, city regen post-wrap, `hostileKills` vs `totalEnemiesKilled` split, `damageEnemy` shield cooldown arming, admin command mutation/shell-effect paths, field-fill spawn-interval stretching, phantom/zapper high-tier unlock reachability, the unified notification queue (idempotence, tick gating, post-expiry promotion), the 3.2.1 worker spook memory window (set on evade-end, per-tick decay, reset on `killWorker`, threat-multiplier flip), the 3.2.1 `archiveLog` routing/cap/migration, and the 3.2.3 `xpForLevel` curve pinned to its sim and HUD consumers.
