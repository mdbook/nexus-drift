...
Current version: **3.2.3**. The in-game changelog is at `src/changelog.ts` and opens via the version badge in the header. As of 2.0.0 the project dropped its leading `0.` prefix from all historical versions — the first release is now `0.1.0` (was `0.0.1`), and the "Living Field" milestone is `2.0.0` (was `0.2.0`).
...
- `src/game/__tests__/advanceGame.test.ts` — 146 tests: simulation invariants, subsystem behavior, achievement edge cases, projectile behavior, corruption linger, worker-slot gating/costs, surround-pressure combat, save/load round-trip, turret/scout/sentinel/city HP, multi-class enemy targeting, missile silo subsystem, worker class abilities (Step 6), worker corruption system including warden parasite latch + uncloak + pin / attach cycle / decay, warden kill credit, node drain, worker reporting, sentinel cleanse, worker reboot, and stepWardenSpawn gates/cooldown semantics (3.1.5 latch expansion)
...
- 199 tests across `src/game/__tests__/advanceGame.test.ts`, `src/game/__tests__/interactionAchievements.test.ts`, `src/game/__tests__/aiBehavior.test.ts`, `src/game/__tests__/adminCommands.test.ts`, and `src/lib/versionCheck.test.ts` cover simulation invariants, interaction achievements, late-game worker-slot gating, worker unlock resource costs, event HUD linger behavior, event modifier composition (multiplicative stacking survives expiry), late-tier event reachability via uncapped `rawTier`, lone-sapper worker reboot (no hp=0 zombie), selectors filtering hp=0 dying enemies from counts/threats, zapper bolts skipping rebooting workers and broken turrets on impact, colonyHealth normalized as hp/maxHp over active workers (corruption-toughness maxHp=150 stays ≤100), cloneGameState isolating its RNG instance, resolveEnemyDeaths idempotency (tickDeathFades owns the single per-tick decrement), late-game defense scoring (focusedBeam/missileLauncher contribute to defenseScore and weightedUpgradeScore), tick-wrap-safe elapsed delta helper and temporary-node expiry across the TICK_WRAP boundary, AI behavior and archetype targeting, flee-direction worker retargeting, crowded-node avoidance, missile grace behavior, corruption linger, surround-pressure combat, live-version polling helpers, admin preview-version generation, manual-override timing, save/load round-trips, multi-class enemy target eligibility, worker corruption + sentinel cleanse (Step 7), warden permanent cloak + 3.1.5 parasite latch (latch-on-contact, uncloak-while-latched, pin-through-worker-movement, release-on-host-death, warden-killed-mid-latch leaves host uncorrupted) + save migration for both cloak and latch fields, city regen post-wrap, `hostileKills` vs `totalEnemiesKilled` split, `damageEnemy` shield cooldown arming, admin command mutation/shell-effect paths, field-fill spawn-interval stretching, and phantom/zapper high-tier unlock reachability.
