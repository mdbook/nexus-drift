...
Current version: **3.1.0**. The in-game changelog is at `src/changelog.ts` and opens via the version badge in the header. As of 2.0.0 the project dropped its leading `0.` prefix from all historical versions — the first release is now `0.1.0` (was `0.0.1`), and the "Living Field" milestone is `2.0.0` (was `0.2.0`).
...
Enemy variety by tier (from `PROGRESSION.combatWeights` in `balance.ts`): wisps appear from tier 0 (no `minTier` gate), raiders from tier 1, rushers tier 3, brutes tier 4, sappers tier 5. Corruptors gate at tier 2; blights are a corruptor variant that replaces the corruptor slot 35% of the time at tier 5+. Leech, phantom, and zapper are gated to tiers 6–7 which exceed the display cap of 5 and are effectively late-game-only. The scout upgrade is hidden until tier 2 (`minTier: 2` in `data.ts`) so it appears exactly when corruptors first become possible.
...
- **raider / wisp**: scout 0.7, city 0.3 — willing to pick off the edges.
