...
Current version: **3.0.0**. The in-game changelog is at `src/changelog.ts` and opens via the version badge in the header. As of 2.0.0 the project dropped its leading `0.` prefix from all historical versions — the first release is now `0.1.0` (was `0.0.1`), and the "Living Field" milestone is `2.0.0` (was `0.2.0`).
...
- Save files carry a `schemaVersion` field (currently `7`). `migrateGameState()` always stamps the current version on load and handles older saves gracefully. The `SCHEMA_VERSION` constant lives in `factories.ts`. v5 added AI fields: `ResourceNode.workTicks`, `Agent.threatMemory`, `Enemy.archetype`, `Enemy.squadId`, optional `Enemy.dashTicks` — all populated with `?? default` fallbacks during migration. v6 (3.0.0 branch in flight) scaffolds the Balancing & Behavior update: new `warden` enemy kind and `missileLauncher` upgrade in the unions; per-worker `speedMod` / `fearMod` / `harvestBias` variance plus class-ability ticks (`overclockTicks`, `sprintTicks`, `sprintCooldown`) and corruption fields (`corrupted`, `corruptionTicks`, `corruptingTicks`, `spottedTicks`, `rebootTicks`); turret HP fields (`hp`, `maxHp`, `damageTicks`, `brokenTicks`); scout/sentinel HP + retreat fields (`hp`, `maxHp`, `damageTicks`, `retreating`, `rebootTicks`); new top-level `missileSilos: MissileSilo[]`, `city: CityState`, `nextSiloId` fields; and stats counters `wardensKilled`, `corruptedPurified`, `turretsBroken`. All new fields carry `?? default` migration fallbacks.
