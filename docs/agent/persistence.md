...
- **v13** (4.0) — adds `state.upgradeAutoMaster: "all" | "none" | "custom"` and `state.upgradeAutoFlags: Partial<Record<UpgradeKey, boolean>>` for the manual/auto purchase split. **Asymmetric default (identity preservation):** fresh 4.0 saves default `upgradeAutoMaster: "none"` + `{}` (players buy manually — the 4.0 identity), but loaded pre-13 saves migrate to `upgradeAutoMaster ?? "all"` + `upgradeAutoFlags ?? {}` so returning players keep the autobuy-everything idle experience. `upgradeAutoFlags` is an object → deep-copied in `cloneGameState`; `upgradeAutoMaster` is a primitive (handled by `...prev`).
  - 4.0 Phase 2 also adds `state.priorityMarks: { enemyId; createdAt }[]` and the optional `Agent.suggestedTarget?: { kind; id?; createdAt }`. Both are short-lived, transient interaction nudges (see [workers.md](workers.md) / [defenses.md](defenses.md)) and are **additive within v13 — no SCHEMA_VERSION bump.** **They are CLEARED on save-load** (4.0.1): `migrateGameState` sets `priorityMarks: []` and each agent's `suggestedTarget: undefined`, since these nudges never need to survive a reload — this also sidesteps any conversion of the 4.0.0 absolute-`expiresAt` field shape to the 4.0.1 `createdAt` shape (see the tick-wrap note below). `cloneGameState` still deep-copies `priorityMarks` (`.map((mark) => ({ ...mark }))`) and rides `suggestedTarget` through the `...agent` shallow spread for the per-tick clone. `createdAt` is the stamp tick; expiry is `elapsedTicks(now, createdAt) >= duration` (wrap-safe, `WORKER_AI.suggestionExpiryTicks` / `PRIORITY_MARK.expiryTicks`).
