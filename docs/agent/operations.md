...
- `src/game/__tests__/interactions.test.ts` (11) — 4.0 soft click-to-suggest: worker suggestion honored when the path is safe, rejected+cleared under path threat, cleared on arrival, cleared on expiry, flee rules still win over a nudge, `suggestWorkerToNode` nearest-eligible selection; defense-priority mark raises turret score without overriding the cloak filter; and the Phase 3 `suggestWorkerHome` (stamps a home marker + drops target, skips fleeing/rebooting, unknown-id guard).
