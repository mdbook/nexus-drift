...
- `src/game/__tests__/interactions.test.ts` (13) — 4.0 soft click-to-suggest: worker suggestion honored when the path is safe, rejected+cleared under path threat, cleared on arrival, cleared on expiry, flee rules still win over a nudge, `suggestWorkerToNode` nearest-eligible selection; defense-priority mark raises turret score without overriding the cloak filter; the Phase 3 `suggestWorkerHome` (drops target with no lingering marker, skips fleeing/rebooting, unknown-id guard); and (4.0.1) wrap-safe `createdAt` expiry for a suggestion and a priority mark stamped just before `TICK_WRAP`.
