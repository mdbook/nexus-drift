...
- `src/game/__tests__/lore.test.ts` (9) — Field Archive lore unlocks: fresh colony fully locked, `hidden` WikiOverlay entries ↔ `LORE_UNLOCKS` keys parity, every `EnemyKind` has a codex entry, unique entry ids, gated entry stays locked until its trigger then unlocks (event / prestige / achievement / lost-drone / full-bestiary), save→load round-trip preserves the derived unlock set, and the guardrail that no `unlockedLore` field is persisted on GameState.
