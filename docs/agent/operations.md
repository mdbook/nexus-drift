...
- `src/game/__tests__/interactions.test.ts` (47) — click-to-suggest: worker suggestion honored when safe, and (4.1.0) RETAINED under a transient path-threat then honored once it clears, cleared on arrival / expiry / node-gone, the Fix 1(a) immediate-retarget (a fresh nudge retargets in one `stepWorkers` tick, not 300+), flee rules still win over a nudge, `suggestWorkerToNode` nearest-eligible selection; defense-priority mark raises turret score without overriding the cloak filter; (4.1.0) forced `suggestWorkerHome` — stamps a persistent `kind:"home"` marker, routes toward `homeX/homeY` and persists until arrival then clears, still flees a threat mid-return, skips fleeing/rebooting, unknown-id guard; the Idle Mode status-indicator helpers (`isIdleModeActive` / `idleModeButtonClass` / `idleModeDotClass` track `upgradeAutoMaster === "all"`); wrap-safe `createdAt` expiry across `TICK_WRAP`; and (4.4.2) the honest tasked-line gating predicate `isSuggestionHonored` (honored only when `agent.target` matches the suggested node id; false for a stamped-but-rejected/pending nudge, no-target, no suggestion, or a non-node `home` suggestion).
...
- `src/lib/leadGesture.test.ts` (3) — `shouldEnterLeadMode` press-and-hold threshold: distinguishes a quick tap (suggest/inspect) from a hold-drag (lead) by time/displacement.
