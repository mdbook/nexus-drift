...
- `src/game/__tests__/purchases.test.ts` (13) — `purchaseUpgrade` success/insufficient/tier-gated/max-level + `enforceGates:false`, and autobuy flag gating (`master="none"` zero buys, `"custom"` skips opted-out upgrades, `"all"` buys cheapest first).
- `src/game/__tests__/interactions.test.ts` (55) — click-to-suggest: worker suggestion honored when safe; (4.5.0) FIRM COMMIT — an ordered node is honored even across a path-threat that the old soft nudge rejected (anti "gold A → gold B"), and the corruption carve-out cancels a non-miner's order when the node becomes heavily corrupted (miners keep it); cleared on arrival / expiry / node-gone, with arrival now gated on the MINING contact radius (25px out retained, 23px within cleared) not the old 26px marker; the Fix 1(a) immediate-retarget (a fresh nudge retargets in one `stepWorkers` tick, not 300+); (4.5.0) a firm order does NOT stop an evading worker from fleeing (evade branch wins, order retained); `suggestWorkerToNode` nearest-eligible selection with (4.5.0) single-owner enforcement (ordering worker B clears A's order on the same node); (4.5.0) cancel affordances — `cancelWorkerOrderToNode` toggles an order off, `cancelWorkerOrder` clears a specific worker's node order; (4.5.1) operator actions are FREE — a nudge / mark / send-home succeeds at energy 0 (nothing deducted, never refused for energy) while the REAL refusals still fire (no eligible worker, no live enemy, a fleeing worker), and energy stays a plain mined resource on a headless run (replaces the removed 4.4.0 energy-cost/refusal tests); defense-priority mark raises turret score without overriding the cloak filter; (4.1.0) forced `suggestWorkerHome` — stamps a persistent `kind:"home"` marker, routes toward `homeX/homeY` and persists until arrival then clears, still flees a threat mid-return, skips fleeing/rebooting, unknown-id guard; the Idle Mode status-indicator helpers (`isIdleModeActive` / `idleModeButtonClass` / `idleModeDotClass` track `upgradeAutoMaster === "all"`); wrap-safe `createdAt` expiry across `TICK_WRAP`; and (4.4.2/4.5.0) the honest tasked-line gating predicate `isSuggestionHonored` (honored only when `agent.target` matches the suggested node id AND the worker is neither evading nor lead-dragged; false for a stamped-but-rejected/pending nudge, no-target, no suggestion, a non-node `home` suggestion, an evading worker, or an active lead-drag).
- `src/game/__tests__/leadPoint.test.ts` (7) — 4.3.0 press-and-hold drag-to-lead: lead-point set/move/clear, workers herd toward the held point, distance falloff, fleeing worker ignores the pull, headless neutrality (`state.leadPoint` never touched on the sim/replay path), and (4.5.1) the FREE/anti-flicker guarantee — a held lead survives a full `advanceGame` tick at energy 0 (never auto-released) and workers still follow it at energy 0 (replaces the removed 4.4.0 energy drain-while-held / auto-release-on-bottom-out tests).
- `src/game/__tests__/lore.test.ts` (9) — archive-lore unlock computation: entries unlock deterministically by progress thresholds, no duplicate/false unlocks, and the unlocked-lore set survives save/load migration.
- `src/lib/versionCheck.test.ts` (7) — flat-version parsing, preview-version generation, semver comparison, `/version` fetch handling.
- `src/lib/leadGesture.test.ts` (4) — `shouldEnterLeadMode` press-and-hold threshold: distinguishes a quick tap (suggest/inspect) from a hold-drag (lead) by time/displacement, plus the 4.5.0 de-twitch assertion (the old 150ms/8px tap stays a tap; thresholds are now 350ms/14px).
- `src/hooks/useTooltip.test.ts` (2) — tooltip hook open/dismiss behavior (hover + tap-to-open, outside-dismiss).
