...
- **Honored** (`isSuggestionHonored(agent)` in `interactions.ts` — a pure predicate, true iff `agent.suggestedTarget?.kind === "node" && agent.target === Number(agent.suggestedTarget.id)`): the subtle **cyan** lead line from the worker to the node **plus** a solid marker ring. The line now draws ONLY in this state, so it always traces a REAL path the worker is on.
- **Pending / not honored** (stamped but the sim keeps rejecting it — the 4.4.1 corruption hard-block for a non-miner, or `pathThreat > suggestionMaxPathThreat` — so `chooseWorkerTarget` retains-but-does-not-apply the suggestion and the worker keeps its own scored target): a distinct **amber, dashed** ring on the node and **no lead line**. This is the "can't reach that yet" feedback — the click visibly registers without drawing a phantom path. When the retained-retry finally lands (path clears / an eligible worker frees up), `agent.target` flips to the suggested id and the cue becomes the cyan line automatically — no separate click needed.

Both cues follow the §Coarse-Pointer FX Budget — rings pulse only at full FX (the pending ring at a gentler low frequency), static under `useLowFxMode` / `reduceFx`. The fix is **presentation-only**: `isSuggestionHonored` is a pure read never called on the sim/headless path, `agent.target` vs `agent.suggestedTarget` are both existing state, so decision-trace neutrality and save schema are untouched. The hard-refusal cues in `App.tsx` `onNodeClick` (no free worker / not enough energy, from `suggestWorkerToNode` returning `false`) are unchanged and cover the different "the nudge was never stamped" case.
