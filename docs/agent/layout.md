...
The 4.3.0 **press-and-hold lead marker** follows the same budget: while `game.leadPoint` is set, `FieldSvg.tsx` draws a cyan **lead ring + center dot** at the held point (a `pointerEvents: none` overlay near the `clickPulse` layer, same hue as the worker→node lead line). Full FX adds a faint dashed **falloff ring** (radius `WORKER_LEAD.falloffRadius`), faint **pull lines** from each currently-led worker (active, alive, non-corrupted, non-rebooting, **non-evading**), and a gentle tick-driven pulse; under `reduceFx` it collapses to the static solid ring + dot (simplify, don't remove). It coexists with the 4.1 cyan worker→node lead line, the 4.x amber marked-enemy ring, and the touch hit-halos — all independent layers.

**Press-and-hold drag-to-lead gesture (4.3.0).** The field `<svg>` carries pointer handlers (`onPointerDown/Move/Up/Cancel`, `onLostPointerCapture`, `onClickCapture`) that implement tap-vs-drag disambiguation without stealing taps. A press promotes to **lead mode** only once it clears the tap threshold — held `LEAD_GESTURE.holdMs` (150) **or** moved `LEAD_GESTURE.moveThresholdPx` (8 screen px) — decided by the pure `shouldEnterLeadMode` helper in `lib/leadGesture.ts` (unit-tested, no DOM). Below the threshold the press stays a plain tap and the existing entity `onClick` handlers (node suggest / inspect) fire unchanged; a **completed** lead gesture sets `swallowClickRef` so `onClickCapture` eats exactly one trailing click (release never also fires a tap). Pointer→world coords go through the SVG's `getScreenCTM().inverse()` (handles viewBox scale + letterboxing). On promote it `setPointerCapture`s so the drag tracks off-target. App wires `onLeadStart` / `onLeadMove` / `onLeadEnd` → `setLeadPoint` / `clearLeadPoint` via `mutateGame`; the handlers no-op when App supplies no lead callbacks. Sim-side movement bias + neutrality live in [workers.md](workers.md) §Press-and-hold Lead.
